...
statistical significance. This revision (Review-2, latency/ML extension)
adds two further mechanisms: a 64-entry LRU lookup cache that measurably
reduces BudgetSym's own lookup latency on hot-path access patterns, and a
workload-adaptive ML threshold predictor (Ridge regression trained offline
on 300 synthetic workloads over a 5-feature online profile) that
reconfigures the policy thresholds after a 100-symbol profiling window with
no additional runtime dependency. Lookup latency overhead remains an
honestly reported cost of the design, now partially mitigated rather than
eliminated.
\end{abstract}
...
statistical significance testing. This revision extends Review-2 along the
axis its own Threats to Validity section flagged as unresolved: lookup
overhead, and adds the data-driven threshold predictor that Review-2's
Future Work section proposed but did not implement.
...
\item A 64-entry, hash-keyed LRU lookup cache that memoizes recently-resolved
  ...
  BUDGET-SYM's lookup latency [\textbf{new}].
...
\item An online WorkloadProfiler (5 features, 100-symbol sampling window)
  ...
  no runtime ML dependency [\textbf{new}].
\end{enumerate}
...
\item Reducing cold-lookup cost directly (e.g., a secondary index keyed by a
...
workload with no runtime ML dependency. Lookup latency overhead is reduced
but remains an honestly reported, unresolved cost relative to the
conventional baseline. Further work toward LLVM integration, real-corpus
predictor training, and a complete prior-art search are the required next
steps.
